...
private:
    ...
    size_type hash(const T & value) const;
...
template <class T, std::size_t TABLE_SIZE>
bool table<T, TABLE_SIZE>::find(const T &value) const
{
...
template <class T, std::size_t TABLE_SIZE>
void table<T, TABLE_SIZE>::insert(const T &value)
{
    if (find(value))
        return;
...
template <class T, std::size_t TABLE_SIZE>
typename table<T, TABLE_SIZE>::size_type table<T, TABLE_SIZE>::hash(const T &value) const
{
    size_type sum(0);
